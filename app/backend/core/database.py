...
class DatabaseManager:
    ...
    async def init_db(self):
        ...
        try:
            ...
            # For asyncpg: strip unsupported connection parameters from the URL
            # Some PostgreSQL configurations include 'channel_binding' which asyncpg
            # doesn't accept as a connect() keyword argument
            if "+asyncpg" in database_url:
                import urllib.parse
                parsed = urllib.parse.urlparse(database_url)
                if parsed.query:
                    params = urllib.parse.parse_qs(parsed.query)
                    # Remove parameters not supported by asyncpg's connect()
                    ...
                    for param in unsupported_params:
                        if param in params:
                            params.pop(param)
                            changed = True
                            logger.info(f"Removed unsupported asyncpg parameter: {param}")
                    if changed:
                        new_query = urllib.parse.urlencode(params, doseq=True)
                        database_url = urllib.parse.urlunparse(parsed._replace(query=new_query))
                        logger.info("Database URL cleaned for asyncpg compatibility")
        ...
        try:
            ...
            try:
                ...
                if "already exists" in str(e):
                    self._initialized = True
                    logger.info(f"Tables already exist (ProgrammingError wrapped by SQLAlchemy): {e}, ignored.")
                else:
